...
mongoose.connect(`mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@clusterfree.neutu.mongodb.net/ProjectDB?retryWrites=true&w=majority`,{ useNewUrlParser: true, useCreateIndex: true, useUnifiedTopology: true, useFindAndModify:false }).then(() => console.log( 'Database Connected' )).catch(err => console.log( err ));
...
app.route("/register")
	.post(function(req,res){
		...
		newUser.save(function(err){
			...
			res.send("200 Ok")
			console.log("/projects/"+newUser.username);
			}
		...
		User.findOne({username:checkUname},(err,found)=>{
			if (err) {
				...
				if(checkPwd===found.password)
					res.send("logged in")
				console.log("/projects/"+checkUname);
		}
		});
	});
...
////////////////////////////////////////////////////Projects/////////////////////////////////////
	app.route("/projects/:uname")
		.get((req,res)=>{
			...
			User.findOne({username:storeUser}).populate('projects').exec(function(err,user){
				if (err) {console.log(err);}
				else{res.send(user.projects);}
...
////////////////////////////////////////////////TASKS///////////////////////////////////////////
		...
		app.route("/tasks/:_pid")
			.get((req,res)=>{
					...
					Project.findOne({_id:idstore}).populate('tasks').exec(function(err,proj){
					if (err) {console.log(err);}
					else{res.send(proj.tasks);}
			});
			...
			.patch((req,res)=>{
				Task.updateOne({_id:req.body._id},{$set:req.body},(err,obj)=>{
					if (err) {console.log(err);}
					else{res.send(obj);console.log("200 OK");}
				});
